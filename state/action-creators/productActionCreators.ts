...
import {
    getAllProducts,
    getProductDetails,
} from '../../rest-client/rest/products';
...
export const loadProductDetails = (id: string) => {
    return async (
        ...
        try {
            ...
            // we will also state this product in recently viewed
            dispatch({
                type: RecentlyViewedProductActionTypes.ADD_PRODUCT_TO_RECENTLY_VIEWED,
                payload: data.product,
            });
            // and state recently viewed piece of state to local storage
            ...
            dispatch({
                ...
                payload: axiosError?.response?.data.error || axiosError.response?.statusText,
            });
        }
    };
};
    ...
    return async (dispatch: Dispatch<ProductsAction>): Promise<void> => {
        ...
        try {
            ...
            dispatch({
                ...
                payload: data.allProducts.sort((a, b) => a.price - b.price),
            });
        } catch (error) {
            const axiosError = error as AxiosError<{ error: any }>;
            dispatch({
