 ...
 * @returns {object} - initial state for cart reducer, or empty object if state is not saved in local storage. Action creators addToCart() and removeFromCart() persist cart into local storage on each call
 */
const cartItemsFromStorage = (): CartState => {
  if (typeof window !== 'undefined') {
    const persistedCart = localStorage.getItem('streetTrendCart');
